...
# Melting
fang_et_al_genotypes <- data.frame(read_tsv("./fang_et_al_genotypes.txt"))
fang <- fang_et_al_genotypes %>% select(-(JG_OTU:Group))
fang <- as.data.frame(t(fang), stringsAsFactors = FALSE)
...
final_melt <- as.data.frame(replace(hetero, hetero == "?/?", "unknown"))

ggplot(data = final_melt) + geom_bar(mapping = aes(x = value), color = "blue", fill = "blue")
